...
import time
import datetime
from ExpMan.core.ExperimentVariable import ExperimentVariable

from ExpMan.core.ExperimentTest import ExperimentTest
...
class ExperimentManager(object):
    ...
    def createExperiment(self, folder, name, info,verbose=True):
        ...
        fout.close()
        fout = open(folder + "/experimentsComments.log","w")
        fout.close()
        fout = open(folder + "/experimentsPostComments.log","w")
        fout.close()
    ...
    def loadExperiment(self, folder, verbose=True):
        if not os.path.isfile(folder + "/general.txt"):
            raise Exception("Not a valid Experiment - general.txt missing")
        if not os.path.isfile(folder + "/experimentsTest.log"):
            raise Exception("Not a valid Experiment - experimentsTest.log missing")  
        fin = open(folder + "/general.txt","r")
        linesIn = fin.readlines()
        fin.close()
        
        if len(linesIn) < 2:
            raise Exception("Not a valid Experiment - general.txt not valid")
            
        self.name = linesIn[0][0:-1]
        
        self.info = ""
        for row in linesIn[1:]:
            self.info += row
    ...
    def addCoding(self, param, code):
        if '_' in code:
            raise Exception("Code shouln't contain '_' character")
        experimentsCoding = open(self.folder + "/experimentsCoding.log", 'a')
        if(not param in self.coder and not code in self.decoder):
            experimentsCoding.write(param + "," + str(code) + "\n")
        else:
            raise Exception("Param or code already present")
        self.coder[param] = code
        self.decoder[code] = param
    ...
    def code(self, params):
        ...
        for p in params[0:-1]:
            try:
                ...
                raise Exception("Parameter not coded")
        try:
    ...
    def decode(self, code):
        ...
        for c in codes:
            try:
                ...
                raise Exception("Code not parametrizable")
        return ret
        
"""class NoExperimentLoaded(Exception):
    def __init__(self, message, errors):
        super(ValidationError, self).__init__("No experiment is loaded. Load or create an Experiment before.")"""